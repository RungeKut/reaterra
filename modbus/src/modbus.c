...
void _error_print(modbus_t *ctx, const char *context)
{
 ...
 //       fprintf(stderr, "ERROR %s", modbus_strerror(errno));
       if (context != NULL) {
           sprintf(b, "ERROR %s: %s", modbus_strerror(errno), context);
 //           fprintf(stderr, ": %s\n", context);
        } else {
            sprintf(b, "ERROR %s", modbus_strerror(errno));
//            fprintf(stderr, "\n");
        }
       logger('E', b);
    }
...
int _modbus_receive_msg(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type)
{
    ...
    if (ctx->debug) {
        if (msg_type == MSG_INDICATION) {
            logger('D', "Waiting for a indication...");
        } else {
        	logger('D', "Waiting for a confirmation...");
        }
    }
    ...
    while (length_to_read != 0) {
        ...
        if (ctx->debug) {
            int i;
            for (i=0; i < rc; i++)
                sprintf(b + (read_cnt++) * 4 ,"<%.2X>", msg[msg_length + i]);
//            for (i = 0; i < msg_length; i++)
//                 sprintf(b + i * 4,"[%.2X]", msg[i]);
     //        printf("\n");
     //        sprintf(b, "GPIO%d written : %d successfully !",ctx->rpi_bcm_pin,value);
...
int modbus_configure_rpi_bcm_pin(modbus_t *ctx, uint8_t value)
{
  ...
  {
	  char b[100];
      sprintf(b, "BCM Pin set as %d", ctx->rpi_bcm_pin);
      logger('D', b);
  }
...
int modbus_rpi_pin_export_direction(modbus_t *ctx)
{
  if(ctx->enable_rpi_rtu == 1)
  {
    ...
    if (-1 == fd)
    {
        if(ctx->debug)
        {
        	logger('E', "Failed to open export for writing!");
        }
        return(-1);
    }
    ...
    if (-1 == fd)
    {
        if(ctx->debug)
        {
        	logger('E', "Failed to open gpio direction for writing!");
        }
        return(-1);
    }
    if (-1 == write(fd, &s_directions_str, 3))
    {
        if(ctx->debug)
        {
        	logger('E', "Failed to set direction!");
        }
        return(-1);
    }
    close(fd);
    if(ctx->debug)
    {
    	logger('D', "RPI pin exported and pin direction configured successfully.");
    }
  }
  return 0;
}
...
int modbus_rpi_pin_unexport_direction(modbus_t *ctx)
{
  if(ctx->enable_rpi_rtu == 1)
  {
    ...
    if(ctx->debug)
    {
    	logger('D', "RPI BCM Pin Unexported successfully.");
    }
  }
  return 0;
}
