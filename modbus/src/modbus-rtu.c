...
#if defined HAVE_DECL_TIOCSRS485 || defined HAVE_DECL_TIOCM_RTS
#include <sys/ioctl.h>
#endif
...
//       if (ctx->debug)
//       {
    	   logger('E', "Failed to open gpio value for writing!");
//       }
       return(-1);
    }
...
//       if (ctx->debug)
//       {
    	   logger('E', "Failed to write value to gpio!");
//       }
       return(-1);
    }
    close(fd);
    ...
    {
//    	char b[200];
      sprintf(b, "GPIO%d written : %d successfully !",ctx->rpi_bcm_pin,value);
      logger('D', b);
    }
    if(value == 0)
    {
...
static int _modbus_rtu_receive(modbus_t *ctx, uint8_t *req)
{
    ...
    if (ctx_rtu->confirmation_to_ignore) {
        ...
        rc = 0;
        if (ctx->debug) {
            logger('D', "Confirmation to ignore");
        }
    } else {
...
//                    rsp[0], req[0]);
//        	char b[200];
          sprintf(b, "The responding slave %d isn't the requested slave %d\n",
                  rsp[0], req[0]);
          logger('E', b);
        }
        errno = EMBBADSLAVE;
                                       ...
                                       const int msg_length)
{
 ...
 //       	char b[200];
          sprintf(b, "Request for slave %d ignored (not %d)", slave, ctx->slave);
          logger('E', b);
        }
...
//        	char b[200];
          sprintf(b, "ERROR CRC received 0x%0X != CRC calculated 0x%0X",
                  crc_received, crc_calculated);
          logger('E', b);
        }
...
//    	char b[200];
      sprintf(b, "Opening %s at %d bauds (%c, %d, %d)",
              ctx_rtu->device, ctx_rtu->baud, ctx_rtu->parity,
              ctx_rtu->data_bit, ctx_rtu->stop_bit);
      logger('I', b);
   }
    ...
    if (ctx->s == -1) {
        if (ctx->debug) {
            sprintf(b, "ERROR Can't open the device %s (%s)\n",
...
//                    ctx_rtu->baud, ctx_rtu->device);
//        	char b[200];
            sprintf(b, "WARNING Unknown baud rate %d for %s (B9600 used)\n",
                    ctx_rtu->baud, ctx_rtu->device);
            logger('E', b);
        }
    }
...
#else
        if (ctx->debug) {
            logger('E', "This function isn't supported on your platform");
        }
        errno = ENOTSUP;
        return -1;
#endif
                              ...
                              struct timeval *tv, int length_to_read)
{
    ...
    while ((s_rc = select(ctx->s+1, rset, NULL, NULL, tv)) == -1) {
        if (errno == EINTR) {
            if (ctx->debug) {
            	logger('E',  "A non blocked signal was caught");
            }
...
modbus_t* modbus_new_rtu(const char *device,
                         ...
                         int stop_bit, void (*log)(char, char*))
{
	logger = log;
    ...
    if (device == NULL || *device == 0) {
    	logger('E', "The device string is empty");
        errno = EINVAL;
        return NULL;
    }
    ...
    /* Check baud argument */
    if (baud == 0) {
    	logger('E', "The baud rate value must not be zero");
        errno = EINVAL;
        return NULL;
    }
